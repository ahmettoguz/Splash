...
/*
 (API command to midfy ModelView(MV) matrix)
	...
	glLoadIdentity(); mv = identity        main i 1 diðerleri 0
	glTranslatef(tx, ty, 0);  mv = mv. (main yine 1   son column = tx, ty, 1) son argüman0: 2 dim olduðu için
	glScalef(sx, sy, 1); mv = mv.(maini sx, sy, 1) diðerleri 0
	glRotatef(Q, 0, 0, 1); mv = mv.([cosq, -sinQ, 0], [sinQ  cosQ 0], [0, 0, 1])
bunlarý yapýnca glvertex2f deðiþiyo tekrar initialize etmek istiyorsan glLoadIdentity(); yi çaðýr.

Ardý ardýna rotate ve translate koydun mesela sondakini, ilk önce yapar yani mesela önce translate sonra rotati yapar çünkü en son oluþan matrixte önce rotate yapýlýr ama translate yapýlýrken sonra uygulanýr.

*/
/*
Stringi deðiþtirmemek için fonksiyona giderken
(const char name[])
*/
#ifdef _MSC_VER
...
void disp_frames()
{
	...
	//frames
	{
		...
		//left					//6 3  //çizginin sýnýrý -735 de
		glVertex2f(-738, 360);
		glVertex2f(-738, -320);

		//right					//6 3  //çizginin sýnýrý -735 de
		glVertex2f(738, 360);
		glVertex2f(738, -320);
